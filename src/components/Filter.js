...
export default class Filter extends Component {
    ...
    submit(){        
        ...
        if(this.state.loc !== ""){
            data = properties.filter(property => property.location.includes(this.state.loc));
        }

        if(this.state.price !== ""){
            data = data.filter(property => property.price <= this.state.price && property.price > this.state.price - 100000);
        }
        ...
        if(this.state.furnishing !== "")
            data = data.filter(property => property.furnishing.includes(this.state.furnishing));
    ...
    render() {
        ...
        const location = [...new Set(properties.map(properties => properties.location))].sort();
        const type = [...new Set(properties.map(properties => properties.type))].sort();
        const furnishing = [...new Set(properties.map(properties => properties.furnishing))].sort();

        return (
            <div className="container d-flex justify-content-center">
                <select  onChange={this.loc_filter}>
                    <option value=""> Location </option>
                    {location.map((loc) => <option key={loc} value={loc}>{loc}</option>)}
                </select>
                ...
                <select  onChange={this.type_filter}>
                    ...
                    {type.map((loc) => <option key={loc} value={loc}>{loc}</option>)}
                </select>
                <select  onChange={this.furnishing_filter}>
                    <option value=""> Furnishing </option>
                    {furnishing.map((loc) => <option key={loc} value={loc}>{loc}</option>)}
                </select>
